...
STEAM education platform; this repository is the backend API.

## Program

Top-level learning track (e.g. robotics). Contains modules, classes, reviews,
and enrollments.

## Module

A stage within a program (`Theory`, `Experiential`, or `Research`). May have
prerequisites, pricing, and retake fees.

## Course

Mentor-owned container within a module holding activities and materials.

## Activity

A single learning task (`SelfPaced`, `LiveOnline`, or `Offline`) inside a
course.

## Class

A cohort (đợt học) running a program on a shared schedule with a mentor.
...
Reusable pool of questions; quizzes may draw random subsets at serve-time.

## Submission

Student attempt record for an assignment; quiz flow uses Pending → Graded
states.

## ApiResult

Standard API response envelope (`isSuccess`, `value`, `error`) used by all
controllers.
