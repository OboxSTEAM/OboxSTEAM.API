...
progress reaches 100% (activities + required assignments). Completing a
program also issues `ProgramCompleted`, and if the learner owns a bundle
containing that program, sequential items with `RequiresPreviousCompletion`
emit `ProgramUnlocked`. When every bundle item is `Completed`, the bundle
enrollment becomes `Completed` and a second **pathway** certificate is issued
(`Certificate.BundleId` set, `ProgramId` null).

Issuance covers program certificates and pathway certificates (`ModuleId` is
null). The API generates a PDF (QuestPDF), uploads it to S3 at
`certificates/{programId}/{studentId}/{code}.pdf` or
`certificates/bundles/{bundleId}/{studentId}/{code}.pdf`, and stores `PdfUrl`
plus a public `VerificationUrl` built from `APP_FRONTEND_URL` / `APP_BASE_URL`.

Endpoints under `/api/certificates`:

- `GET /me` — list certificates for the current user scope (program and bundle)
...
List/detail DTOs expose `programId` **or** `bundleId`. `GET /me` returns both
kinds. The FE owns share UI and PDF download UX using `pdfUrl` and
`verificationUrl`. Skills and learning outcomes come from existing
`Program.SkillsGained` and module `LearningOutcomes` text arrays.
