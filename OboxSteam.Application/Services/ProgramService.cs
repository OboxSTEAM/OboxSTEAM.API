...
public class ProgramService : IProgramService
{
    ...
    public async Task<ProgramCurriculumDto> GetProgramCurriculumAsync(Guid id)
    {
        ...
        if (program == null || program.IsDeleted)
        {
            _logger.LogWarning("[GetProgramCurriculumAsync] Program with Id {Id} not found.", id);
            throw ErrorHelper.NotFound($"Program with id '{id}' not found.");
        }
        ...
        var materials = allActivityIds.Count > 0
            ? await _unitOfWork.Materials.GetAllAsync(
                m => allActivityIds.Contains(m.ActivityId) && !m.IsDeleted)
            : new List<Material>();

        var materialsByActivityId = materials.ToDictionary(m => m.ActivityId);
        ...
        var moduleDtos = modules.Select(module =>
        {
            ...
            {
                moduleDto.Milestones = milestonesByModuleId.TryGetValue(module.Id, out var moduleMilestones)
                    ? moduleMilestones.Select(milestone => new ProgramCurriculumMilestoneDto
                    {
                        MilestoneId = milestone.Id,
                        MilestoneName = milestone.Title,
                        MilestoneOrder = milestone.MilestoneOrder,
                        Activities = milestoneLinksByMilestoneId.TryGetValue(milestone.Id, out var links)
                            ? links
                                .Select(link => activitiesById.GetValueOrDefault(link.ActivityId))
                                .Where(activity => activity != null)
                                .Select(activity => MapCurriculumActivity(activity!, materialsByActivityId))
                                .ToList()
                            : new List<ProgramCurriculumActivityDto>(),
                    }).ToList()
                    : new List<ProgramCurriculumMilestoneDto>();
            }
            else if (coursesByModuleId.TryGetValue(module.Id, out var moduleCourses))
            {
                ...
                moduleDto.Courses = moduleCourses.Select(course => new ProgramCurriculumCourseDto
                {
                    ...
                    Activities = activitiesByCourseId.TryGetValue(course.Id, out var moduleActivities)
                        ? moduleActivities
                            .Select(activity => MapCurriculumActivity(activity, materialsByActivityId))
                            .ToList()
                        : new List<ProgramCurriculumActivityDto>(),
    ...
    private static ProgramCurriculumActivityDto MapCurriculumActivity(
        Activity activity,
