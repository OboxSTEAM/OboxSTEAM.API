...
public static class JwtUtils
{
    ...
    public static string GenerateActionToken(Guid userId, string purpose, string? extraData, IConfiguration configuration, TimeSpan validityPeriod)
    {
        var secretKey = configuration["JWT:SecretKey"];
        var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(secretKey!));
        var signingCredentials = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);
        ...
        var claims = new List<Claim>
    {
        ...
        {
            claims.Add(new Claim("ExtraData", extraData));
        }
    ...
    public static ClaimsPrincipal ValidateActionToken(string token, IConfiguration configuration)
    {
        var secretKey = configuration["JWT:SecretKey"];
        var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(secretKey!));
